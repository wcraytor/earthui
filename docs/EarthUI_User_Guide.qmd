...
\item \textbf{\textcolor{earthgreen}{Variable Selection}}
  \begin{itemize}
    ...
    \item Advanced: Prolog Remarks Processing (Optional)
...
\begin{tcolorbox}[colback=earthgreen!5, colframe=earthgreen!40, title=]
\small\textit{\textbf{Tip:} Review the T/F/NA column in the predictor table after import (the third number is the NA count). Columns with more than 30\% missing values are flagged in red. Consider excluding high-NA columns or cleaning the data before import.}
\end{tcolorbox}
...
**2. Import Data** --- Select a data file from the active project's input folder (CSV or Excel). For Excel files with multiple sheets, a sheet selector appears. Column names are automatically converted to snake\_case.
...
\hline
Residual Contribution & Estimates the market contribution of rare or excluded features (e.g. a workshop present in only 3 sales) by regressing model residuals on columns the model never used. See \textit{Residual Contribution (Rare-Feature Estimation)} below. \\
\hline
...
Some value-bearing features are too rare for the regression channel: a
workshop present in 3 of 780 sales cannot earn a defensible coefficient, and
the forward pass will usually pass it over entirely. In the RCA two-channel
system such features belong to the \textbf{residual layer}, and the
\textbf{Residual Contribution} tab prices them there.

\textbf{Method.} The model is fit \emph{without} the feature (either
excluded in the predictor table, or offered but never selected by the forward
pass --- both qualify). Its value then sits in the residuals of the flagged
sales, and the tab regresses the model's residuals on the selected columns by
ordinary least squares. A binary flag yields a lump-sum contribution; a
numeric column (e.g. \texttt{outbuilding\_sqft}) yields a per-unit
contribution.

\textbf{Select correlated amenities together.} With several columns selected
(e.g. \texttt{outbuilding\_sqft} \emph{plus} \texttt{pr\_has\_workshop})
the regression is joint, so overlapping amenities split the credit instead of
each claiming the same dollars --- essential when the flagged sales overlap.

\textbf{Reading the output.} \texttt{estimate} is the contribution;
\texttt{std\_error} is one standard error (estimate $\pm$ 2\,SE $\approx$
95\% band; $t \geq 2$ is the usual ``probably real'' yardstick). For each
binary flag the tab lists the flagged sales' individual residuals --- read
these as the receipts: consistent same-sign residuals support the estimate,
while one outlier carrying it means discount heavily. Terms with $n < 10$
get an automatic \textsc{caution} in the generated workfile note; corroborate
them with depreciated cost or a remarks-widened sample.

\textbf{Selection discipline.} (1) cluster correlated amenities in one run;
(2) exclude proxies of predictors the model used (the tab refuses features
the model actually used); (3) keep the column set small and causally
plausible; (4) verify flag coverage --- unflagged workshops in the control
pool dilute the estimate; (5) audit the flagged-sale residuals before
believing the estimate.

The generated \textbf{workfile note} documents method, estimates, standard
errors, and cautions in one paragraph suitable for the report and the
residual breakdown documentation.
...
| **T/F/NA** | Three counts per column: logicals show TRUE/FALSE/NA, numerics show $\neq$0/$=$0/NA, other types show 0/0/NA. Shown in red when more than 30\% of values are missing |
...
This feature is **off by default** and entirely optional --- earthUI's normal workflow does not depend on it, and it assumes working knowledge of Prolog. Enable it via **Settings (gear icon) $\rightarrow$ Enable Prolog / list column processing**; the choice is remembered per project.
...
**Market area profiles (climate-region priors).** Three R helpers --- `climate_region_for()`, `market_area_profile()`, and `climate_feature_priors()` --- classify a project's location (county + city) into a buyer-recognized climate region and return ordinal contributory-value priors for HVAC/cooling features (heating, A/C, swamp cooler, whole-house fan, filtration) by region. A Prolog module (`inst/prolog/climate_regions_ca.pl`) mirrors the same classification for use inside derivation rules. These priors are experimental and intended to inform valuation of thin-data features and sanity-check estimated coefficients; region assignments should be reviewed by the appraiser.

**California is the first jurisdiction, not the boundary.** The shipped knowledge covers California's seven buyer-recognized climate regions (reduced from the California Energy Commission's official 16 building-climate zones), but the architecture is deliberately international: climate knowledge is being restructured as **jurisdiction packs** --- self-contained, data-shaped fact tables (zones, admin-unit assignments, feature priors) keyed to regProj's country and admin-level codes, so a pack can describe a U.S. state, a German Land, or a small country outright. Packs for new jurisdictions, and **market-area modules** capturing local vocabulary and priors for a named market area, are designed to be authored *in the field* by Valuation Engineers and appraisers --- in their own language, and typically with LLM assistance (the pack format is a documented schema with a worked example, expressly so that an assistant like Claude can draft a valid pack from a plain-language description; nobody needs to write Prolog, which earthUI generates from the pack). Every field-authored pack must carry provenance --- author of record, date, and the authority cited --- and passes a blocking validator (schema, coverage, referential-integrity, and dry-run checks) before it can be enabled, so the responsible professional, not the drafting tool, stands behind the knowledge. See `docs/climate_packs_design.md` in the repository for the full design.
...
More folds train each fold on more data (a less biased estimate) but shrink the held-out test fold. The table graduates up to the standard 10-fold as the sample grows while keeping each test fold large enough ($\approx$50+ rows) for a stable per-fold metric, and falls back to 5-fold for small samples. It caps at 10 --- beyond that adds compute (each fold is a full earth fit) for negligible gain. For small samples the recommendation also suggests raising ncross (e.g. 15--20) to stabilise the CVR\textsuperscript{2} estimate. With `pmethod = "backward"`, cross-validation does not affect the model --- it only computes the diagnostic CVR\textsuperscript{2} and provides residuals for the variance model.
...
Repeats average out fold-partition luck to stabilise CVR\textsuperscript{2}; larger samples need fewer repeats because a single CV pass is already more stable.
...
After every successful fit, earthUI writes `<filename>_earth_output_<timestamp>.txt` to the output folder. This file contains the model terms, summary statistics (R\textsuperscript{2}, GRSq, CVRSq), variance model details, and trace log. One file is created per fit, providing a cumulative record for comparing parameter configurations.
...
| `cqa` | Condition-Quality-Appeal score (2 dp, range 0--10) |
...
On every successful fit with `degree` $\leq$ 2, earthUI automatically saves the full result object as an `.rds` file to the project's output folder. The filename follows the pattern `<datafile>_earthUI_result_<YYYYMMDD_HHMMSS>.rds`.
...
\item Create (or open) a project with purpose \textbf{For Appraisal}
\item Copy \texttt{Appraisal\_1.csv} into the project's input folder and pick it in Section 2
