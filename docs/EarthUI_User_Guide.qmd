...
by William B. Craytor, Pacific Vista Net
\end{center}
...
Reports are rendered via Quarto and saved directly to the Project Output Folder as `<datafile>_report_<YYYYMMDD_HHMMSS>.<format>`. Rendering runs in the background --- a modal dialog shows elapsed time and Quarto progress while the app remains responsive. All plots and data are pre-generated after each successful fit, so switching between formats is fast (no R re-computation). A notification confirms the file location on completion.
