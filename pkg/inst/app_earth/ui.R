          ...
          tabPanel("Residual Contribution", br(),
            div(style = "max-width: 900px;", class = "rescontrib-box",
              ...
              p(class = "text-muted",
                ...
                "WITHOUT, by jointly regressing model residuals on them ",
                "(RCA residual method). Binary flag -> lump sum; numeric -> per unit. ",
                "Select several correlated amenities together so the credit is split, ",
                "not double-counted."),
              selectizeInput("rescontrib_features", "Feature column(s) (not in model)",
                             choices = NULL, multiple = TRUE, width = "100%"),
              actionButton("rescontrib_go", "Estimate contributions",
                           class = "btn-primary"),
              br(), br(),
              DT::dataTableOutput("rescontrib_table"),
              uiOutput("rescontrib_flagged"),
              h5("Workfile note"),
              verbatimTextOutput("rescontrib_note")
            )
          ),
          tabPanel("ANOVA", br(),
            div(class = "eui-tab-waiting", style = "text-align: center; padding: 60px 20px;",
