...
earthUI works on macOS, Windows, and Linux with R >= 4.1.0. All features
...
earthUI displays recommended values below key parameters in the sidebar.
These update reactively based on the number of fitting rows (*n*) and
selected predictors (*p*). Key recommendations:
...
| **nfold** | banded by sample size: 5 (n < 300) rising one fold per 100 rows to 10 (n >= 700) | 5 |
| **ncross** | banded by sample size: 20 (n < 200) falling to 5 (n >= 3000) | 16 |
| **varmod.method** | lm | lm |
| **newvar.penalty** | 0.1 (if collinear predictors) | 0.1 |

The nfold and ncross recommendations come from banded sample-size tables
rather than a single formula: small samples get fewer folds (so each test
fold stays large enough for a stable metric) but more CV repetitions (to
average out fold-partition noise), while large samples get the standard
10-fold with fewer repetitions. A variance model (`varmod.method`) requires
`nfold > 1` and `ncross > 1`. The remaining formulas are derived from
Friedman's MARS paper, earth's C source code, and empirical testing. See
the earthUI User Guide, Chapter 7 for detailed explanations and scaling
tables.
...
In the Shiny app, report rendering runs in the background --- the UI
stays responsive while the report is generated, and a modal dialog
shows progress.
