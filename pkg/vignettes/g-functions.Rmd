...
- **region** (North / South / West): the plant's climate zone. Climate
  ...
  throughput:
    - *North*: cold winters drive the highest base cost (supplemental
      space heating, winter pipeline surcharges). However, the Northern
      plants were built with heavy-duty infrastructure — oversized
      boilers, redundant compressors, high-capacity gas feeds — so
      marginal costs stay low when throughput increases.
    - *South*: warm climate gives the lowest base cost. However, the
      Southern plants have lighter infrastructure (smaller gas feeds,
      no on-site fuel storage, fewer night-shift contractors), so
      marginal costs escalate steeply when gas or electric draw
      exceeds normal levels.
    - *West*: moderate climate and mid-range infrastructure place
      these plants between North and South for both base and marginal
      costs.
...
- *Gas × region*: per-gallon fuel costs rise steeply in the South
  because the lighter gas infrastructure hits capacity limits sooner.
  Northern plants, with heavy-duty pipeline connections, absorb
  increased gas usage at lower marginal cost.
...
- *Region × shift*: the night-shift premium varies dramatically by
  ...
  Western plants face a moderate Night premium, while Northern plants
  (already staffed for round-the-clock winter operations) add only a
  small Night increment.
- *Gas × electricity × region*: when both gas and electric draw are
  ...
  demand most aggressively. Northern plants, with heavy-duty
  infrastructure, see only a small three-way premium.
- *Gas × region × shift*: the per-gallon gas surcharge on Night shifts
  ...
  night). Western Night shifts face a moderate premium.
- *Product × region × shift*: certain product-region-shift
  ...
  in warm ambient air at peak nighttime electricity rates. Dried
  product at a Southern Night shift also incurs a premium because the
  drying ovens must run longer in humid conditions with no on-site gas
  reserves.
...
df$cost <- 2000 +
  ...
  ifelse(df$region == "North", 0.05, ifelse(df$region == "South", 0.5, 0.25)) *
    pmax(0, df$gas_gallons - 12) * pmax(0, df$electric_kwh - 250) +
  (ifelse(df$region == "South" & df$shift == "Night", 30,
     ifelse(df$region == "West" & df$shift == "Night", 25, 0))) *
    pmax(0, df$gas_gallons - 10) +
  ifelse(df$product == "Frozen" & df$region == "South" & df$shift == "Night", 800,
    ifelse(df$product == "Frozen" & df$region == "West" & df$shift == "Night", 400,
      ifelse(df$product == "Dried" & df$region == "South" & df$shift == "Night", 300,
        ifelse(df$product == "Dried" & df$region == "West" & df$shift == "Night", 150, 0)))) +
  rnorm(n, 0, 30)
...
West Night shows a moderate premium; other combinations contribute zero
because the main effects already account for them.
...
draw are high simultaneously. The West panel shows a moderate effect.
The North panel appears flat because North is the **reference level**
in the earth model's factor encoding: the model expresses interaction
corrections *relative to North*, so North's three-way contribution is
zero by construction. North's actual three-way cost is captured in the
non-factor 3rd-degree g-function above.
...
The two Night-shift lines (dashed) rise steeply with gas consumption:
South-Night (green-dashed) shows the largest per-gallon surcharge
because Southern plants must arrange emergency off-site gas deliveries
for Night shifts. West-Night (blue-dashed) shows a similar but
slightly smaller premium. All Day-shift lines (solid) and
North-Night sit at zero — their three-way interaction is fully
captured by the lower-order terms (main effects and two-way
interactions).
...
combinations: Frozen product at a Southern plant ($835) tops the
chart because blast-freezer compressors fighting warm ambient air at
peak nighttime rates compound all three factors simultaneously.
Frozen-West-Night ($339) and Dried-South-Night ($387) show smaller
but substantial premiums. Canned product and Northern plants show
zero because their three-way interaction is fully captured by the
lower-order terms.
